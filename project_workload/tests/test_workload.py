...
class TestWorkload(TestWorkloadCommon):
    ...
    def test_change_user(self):
        task = self._create_task()
        task.user_ids = [(6, 0, [self.user_2.id])]
        self.assertEqual(task.workload_ids.user_id, self.user_2)
        self.assertEqual(task.workload_ids.unit_ids.user_id, self.user_2)

    def test_add_user(self):
        task = self._create_task()
        task.user_ids = [(4, self.user_2.id)]
        self.assertEqual(len(task.workload_ids), 2)
        self.assertEqual(task.workload_ids.mapped("user_id"), self.user_1 + self.user_2)
        self.assertEqual(
            task.workload_ids.unit_ids.mapped("user_id"), self.user_1 + self.user_2
        )

    def test_remove_user(self):
        task = self._create_task()
        task.user_ids = [(5, 0, 0)]
        self.assertFalse(task.workload_ids)

    def test_change_date(self):
        task = self._create_task()
        task.planned_date_end = task.planned_date_start + timedelta(days=13)
        workload = task.workload_ids
        self.assertEqual(len(workload), 1)
        self.assertEqual(workload.date_start, task.planned_date_start.date())
        self.assertEqual(workload.date_end, task.planned_date_end.date())
