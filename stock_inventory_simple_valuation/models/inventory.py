...
class StockInventoryLine(models.Model):
    ...
    def _search_cost_supplierinfo(self):
        ...
        if sup_info and sup_info[0].price:
            ...
            self.origin_record_reference = "product.supplierinfo,%s" % sup_info[0].id
            self.origin_record_name = sup_info[0].name.name
            return True
        return False

    def _search_cost_invoice_lines(self):
        most_recent_invoice_line = self.env["account.move.line"].search(
            [
        ...
        if most_recent_invoice_line:
            ...
            self.origin_record_reference = (
                "account.invoice,%s" % most_recent_invoice_line.move_id.id
            )
            self.origin_record_name = most_recent_invoice_line.move_id.name
            return True
        return False
    ...
    def _search_cost_po_lines(self):
        ...
        if po_line:
            ...
            self.origin_record_reference = "purchase.order,%s" % po_line.order_id.id
    ...
    def _search_cost_standard_price(self):
        if self.product_id.standard_price:
            ...
            self.origin_record_reference = "product.product,%s" % self.product_id.id
