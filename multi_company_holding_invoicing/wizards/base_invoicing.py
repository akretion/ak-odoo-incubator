...
class ChildInvoicing(models.TransientModel):
    ...
    @api.model
    def _link_sale_order(self, invoice, sales):
        sales.write({"invoice_ids": [(6, 0, [invoice.id])], "state": "done"})
    ...
    def _prepare_invoice_line(self, data_line):
        ...
        if data_line.get("name") == "royalty":
            ...
            val_line = super(ChildInvoicing, self)._prepare_invoice_line(data_line)
        if data_line.get("__domain"):
            order_ids = self.env["sale.order"].search(data_line["__domain"]).ids
            line_ids = (
                self.env["sale.order.line"].search([("order_id", "in", order_ids)]).ids
            )
