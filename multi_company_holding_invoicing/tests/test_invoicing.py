...
class TestInvoicingFromSaleTeam(CommonGenerateInvoice):
    ...
    def _start_scenario(
        ...
        self.child_invoice_a = invoice.child_invoice_ids.filtered(lambda s: s.company_id == self.company_a)
        self.child_invoice_b = invoice.child_invoice_ids.filtered(lambda s: s.company_id == self.company_b)
