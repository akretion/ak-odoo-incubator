...
class CommonInvoicing(SavepointCase):
    ...
    def _get_account_id(cls, company, name):
        return (
            ...
            .search(
                [("name", "ilike", "Product Sales"), ("company_id", "=", company.id)]
            )[0]
            .id
        )
    ...
    def _configure_product_and_taxe(cls):
        ...
        for company in [cls.company_a, cls.company_b, cls.company_holding]:
            # configure accounts
            account_vals = {
                "property_account_income": cls._get_account_id(
            ...
            holding_royalty_product.with_context(force_company=company.id).write(
                account_vals
            )
            ...
            env["account.fiscal.position"].create(
                {
                    ...
                    "tax_ids": [
                        (
                            0,
                            0,
                            {"tax_src_id": sale_tax_a.id, "tax_dest_id": sale_tax_b.id},
                        ),
                        (
                            0,
                            0,
                            {
                                "tax_src_id": purchase_tax_a.id,
                                "tax_dest_id": purchase_tax_b.id,
                            },
                        ),
                    ],
                }
    ...
    def _get_sales(cls, xml_ids):
        ...
        for xml_id in xml_ids:
            sale = cls.env.ref("multi_company_holding_invoicing.sale_order_%s" % xml_id)
            sales |= sale
        return sales
    ...
    def _check_number_of_invoice(self, invoices, number):
        self.assertEqual(
            len(invoices),
            1,
            msg="Only one invoice should have been created, %s created" % len(invoices),
        )
