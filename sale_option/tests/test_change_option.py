...
class TestOptionCase(TransactionCase):
    def test_sale_line_change(self):
        ...
        new_product = self.env.ref(
            'sale_option.product_pc_assmb_option2').id
        ...
        bom_opt2 = self.env.ref('sale_option.bom_pc_assmb_option2')
    ...
    def test_check_option_line_when_bom_change(self):
        "Check if remove bom.line have no impact on optional bom lines"
        bom_line = self.env.ref('sale_option.bom_line_pc_assmb_option13')
        product_id = bom_line.product_id.id
        bom_line.unlink()
        option = [x for x in self.sale_line.optional_bom_line_ids
    ...
    def setUp(self):
        ...
        self.sale_line = self.env.ref('sale_option.sale_order_line_option')
